      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-20">
          ...
          <div className="md:col-span-8">
            <h3 className={`text-4xl md:text-6xl font-bold tracking-tighter leading-tight ${darkMode ? 'text-white' : 'text-black'}`}>
              {lang === 'id' ? 'Umpan balik dari mitra dan rekan kolaborasi.' : 'Feedback from partners and industry leaders.'}
            </h3>
          </div>
        </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-px ${darkMode ? 'bg-white/10' : 'bg-black'}`}>
          {portfolioData.testimonials.map((item) => {
            const contentText = typeof item.content === 'object' ? (item.content[lang] || item.content.en) : item.content;
            return (
              <div
                key={item.id}
